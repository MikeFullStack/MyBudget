  ...
  template: `
    <div class="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
      <div class="bg-white dark:bg-gray-900 rounded-3xl p-6 w-full max-w-sm shadow-2xl transition-colors duration-300">
        <h3 class="text-xl font-bold mb-4 text-gray-900 dark:text-white">Créer un budget</h3>
        <div class="space-y-4">
          <input type="text" [(ngModel)]="name" class="w-full bg-gray-100 dark:bg-gray-800 dark:text-white rounded-xl px-4 py-3 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-black dark:focus:ring-white transition-colors" placeholder="Nom">
          ...
          <p class="text-xs text-gray-500 dark:text-gray-400 px-1">
              {{ type === 'wallet' ? 'Gérez vos transactions au quotidien.' : 'Planifiez votre salaire et vos charges fixes.' }}
          </p>
        ...
        <div class="flex gap-2 mt-6">
          ...
          <button (click)="submit()" [disabled]="!name" class="flex-1 px-4 py-3 rounded-xl bg-black dark:bg-white text-white dark:text-black font-semibold hover:bg-gray-800 dark:hover:bg-gray-200 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">Créer</button>
        </div>
      </div>
    </div>
  `
