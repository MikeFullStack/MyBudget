  ...
  template: `
    <div class="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
      <div class="bg-white dark:bg-gray-900 rounded-3xl p-6 w-full max-w-sm shadow-2xl transition-colors duration-300">
        <h3 class="text-xl font-bold mb-4 text-gray-900 dark:text-white">{{ 'modal.goal.title' | translate }}</h3>
        
        <div class="space-y-4">
          <div>
            <label class="block text-xs font-semibold text-gray-400 mb-1 ml-1">{{ 'modal.create_budget.name' | translate }}</label>
          ...
          <div>
             <label class="block text-xs font-semibold text-gray-400 mb-1 ml-1">{{ 'modal.goal.target' | translate }} ($)</label>
          ...
          <div>
             <label class="block text-xs font-semibold text-gray-400 mb-1 ml-1">{{ 'modal.goal.icon' | translate }}</label>
        ...
        <div class="flex gap-2 mt-6">
          ...
          <button (click)="submit()" [disabled]="!name || !target" class="flex-1 px-4 py-3 rounded-xl bg-black dark:bg-white text-white dark:text-black font-semibold hover:bg-gray-800 dark:hover:bg-gray-200 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">{{ 'modal.create_budget.create' | translate }}</button>
        </div>
      </div>
    </div>
  `
